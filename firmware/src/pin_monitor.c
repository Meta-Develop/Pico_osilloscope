...
uint64_t pin_monitor_sample_interval_ps(void) {
    ...
    double rate_hz = (double)system_clock_hz /
                     ((double)PIO_SAMPLE_LOOP_CYCLES * (double)clock_divider);
...
static void dma_irq_handler(void) {
    if (dma_chan >= 0 && (dma_hw->ints0 & (1u << dma_chan))) {
        ...
        dma_target_start_ps = batch_start_ps +
            ((uint64_t)PIN_BUFFER_SIZE * pin_monitor_sample_interval_ps());
...
uint32_t pin_monitor_read(uint32_t *buffer,
    ...
    source = (const uint32_t *)read_buffer;
    ...
    for (uint32_t i = 0; i < to_read; i++) {
        buffer[i] = source[i] & pin_mask_config;
    }
    ...
    if (sample_interval_ps != NULL) {
        *sample_interval_ps = pin_monitor_sample_interval_ps();
    }
